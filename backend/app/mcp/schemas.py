...
class CompleteTaskSchema(BaseModel):
    """Schema for complete_task MCP tool"""
    user_id: int = Field(description="ID of the user completing the task")
    task_id: int = Field(description="ID of the task to mark as complete", gt=0)


class DeleteTaskSchema(BaseModel):
    """Schema for delete_task MCP tool"""
...
class UpdateTaskSchema(BaseModel):
    """Schema for update_task MCP tool"""
